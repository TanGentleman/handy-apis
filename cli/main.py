...
New agent-ready commands:
- load: Download docs to ~/.docpull/
- list: Show available collections from API
- status: Show what's loaded locally
- chat: Set up agent environment (local or cloud)
"""
...
# Lazy-load API_BASE to allow --help without config
...
app = typer.Typer(
    help="Agent-ready documentation CLI. Download, manage, and use docs with coding agents.",
...
@app.command()
def load(
    collections: Annotated[list[str], typer.Argument(help="Collection IDs to download (e.g., modal fasthtml)")],
    force: Annotated[bool, typer.Option("--force", "-f", help="Re-download even if exists")] = False,
):
    """Download documentation to ~/.docpull/ for agent use.

    Examples:
        docpull load modal           # Download Modal docs
        docpull load modal fasthtml  # Download multiple collections
        docpull load modal --force   # Force re-download
    """
    from cli.store import download_collection, collection_exists, get_collection_path

    for collection in collections:
        try:
            if collection_exists(collection) and not force:
                path = get_collection_path(collection)
                print(f"{collection}: Already loaded at {path}")
                print(f"  Use --force to re-download", file=sys.stderr)
                continue

            print(f"{collection}: Downloading...", file=sys.stderr)
            path, stats = download_collection(collection, force=force)
            print(f"{collection}: {stats['total']} pages -> {path}")

        except httpx.HTTPStatusError as e:
            if e.response.status_code == 404:
                print(f"{collection}: Not found (check 'docpull list' for available)", file=sys.stderr)
            else:
                handle_http_error(e)
...
def list_collections():
    """List available collections from the API.

    Shows all documentation sites that can be loaded with 'docpull load'.
    """
    from cli.store import get_available_sites, list_local_collections
    ...
    # Get local collections for comparison
    local = {c["id"] for c in list_local_collections()}

    print("Available collections:\n")
    for site in sites:
        site_id = site["id"]
        marker = " [loaded]" if site_id in local else ""
        print(f"  {site_id}{marker}")

    print(f"\nTotal: {len(sites)} collections")
    print("\nUse 'docpull load <collection>' to download")


@app.command()
def status():
    """Show what's loaded in ~/.docpull/.

    Displays all locally available documentation collections with metadata.
    """
    from cli.store import list_local_collections, get_collection_stats, DOCPULL_HOME

    collections = list_local_collections()

    if not collections:
        print(f"No collections loaded yet.")
        print(f"\nRun 'docpull list' to see available collections")
        print(f"Run 'docpull load <collection>' to download")
        return

    print(f"Documentation store: {DOCPULL_HOME}\n")
    print(f"{'COLLECTION':<20} {'PAGES':<10} {'SIZE':<12} {'LOADED'}")
    print("-" * 60)
    ...
    for coll in collections:
        ...
        pages = str(stats.get("pages", "?"))
        size_bytes = stats.get("total_size", 0)
        size = f"{size_bytes / 1024:.1f} KB" if size_bytes < 1024 * 1024 else f"{size_bytes / 1024 / 1024:.1f} MB"
        loaded = stats.get("loaded_at", "?")[:10]
        status_marker = "" if stats.get("exists") else " [missing]"

        print(f"{coll['id']:<20} {pages:<10} {size:<12} {loaded}{status_marker}")
...
@app.command()
def update(
    collection: Annotated[Optional[str], typer.Argument(help="Collection to update (or all if omitted)")] = None,
):
    """Refresh loaded collections from the API.

    Re-downloads the documentation to get the latest content.
    """
    from cli.store import download_collection, list_local_collections

    if collection:
        collections = [collection]
    else:
        collections = [c["id"] for c in list_local_collections()]

    if not collections:
        print("No collections to update. Use 'docpull load' first.", file=sys.stderr)
        raise typer.Exit(1)

    for coll in collections:
        try:
            print(f"{coll}: Updating...", file=sys.stderr)
            path, stats = download_collection(coll, force=True)
            print(f"{coll}: {stats['total']} pages updated")
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 404:
                print(f"{coll}: Not found on server", file=sys.stderr)
            else:
                print(f"{coll}: Error - HTTP {e.response.status_code}", file=sys.stderr)
        except Exception as e:
...
@app.command()
def remove(
    collection: Annotated[str, typer.Argument(help="Collection to remove")],
):
    """Remove a collection from ~/.docpull/.

    Deletes the local documentation folder.
    """
    from cli.store import delete_collection, collection_exists

    if not collection_exists(collection):
        print(f"Collection '{collection}' not found locally", file=sys.stderr)
        raise typer.Exit(1)

    if delete_collection(collection):
        print(f"Removed {collection}")
    else:
        print(f"Failed to remove {collection}", file=sys.stderr)
        raise typer.Exit(1)


@app.command()
def chat(
    collection: Annotated[str, typer.Argument(help="Collection to use for chat")],
    cloud: Annotated[bool, typer.Option("--cloud", help="Use cloud sandbox (Modal + OpenCode)")] = False,
    stop: Annotated[bool, typer.Option("--stop", help="Stop running sandboxes")] = False,
):
    """Set up an agent environment with documentation.

    Local mode (default):
        Creates symlink ./docs/<collection> -> ~/.docpull/<collection>
        Generates ./DOCS.md with context for your agent

    Cloud mode (--cloud):
        Uploads docs to Modal Volume
        Creates Modal Sandbox with OpenCode
        Returns WebUI URL and connection info

    Examples:
        docpull chat modal          # Local: symlink + DOCS.md
        docpull chat modal --cloud  # Cloud: Modal Sandbox with OpenCode
    """
    from cli.store import collection_exists

    if not collection_exists(collection):
        print(f"Collection '{collection}' not found.", file=sys.stderr)
        print(f"Run 'docpull load {collection}' first.", file=sys.stderr)
        raise typer.Exit(1)

    if cloud:
        # Cloud mode with Modal Sandbox
        from cli.chat import setup_cloud_chat

        try:
            result = setup_cloud_chat(collection)
        ...
        print("\n" + "=" * 60)
        print("CLOUD CHAT ENVIRONMENT READY")
        print("=" * 60)
        print(f"\nCollection: {result['collection']}")
        print(f"Docs path:  {result['docs_path']}")
        print(f"\nWebUI:      {result['web_url']}")
        print(f"Username:   {result['username']}")
        print(f"Password:   {result['password']}")
        print(f"\nSandbox ID: {result['sandbox_id']}")
        print(f"\nCommands:")
        print(f"  TUI:   {result['commands']['attach_tui']}")
        print(f"  Shell: {result['commands']['shell']}")
        print("=" * 60)
    else:
        # Local mode with symlink
        from cli.chat import setup_local_chat

        try:
            result = setup_local_chat(collection)
        except ValueError as e:
            print(f"Error: {e}", file=sys.stderr)
            raise typer.Exit(1)

        print(f"\nLocal chat environment ready!")
        print(f"\n  Docs:    {result['symlink']}")
        print(f"  Source:  {result['source']}")
        if result.get("docs_md"):
            print(f"  Context: {result['docs_md']}")
        print(f"\nRun your agent in this directory to use the docs.")


# ===========================================================================
# ORIGINAL COMMANDS (for backward compatibility and advanced use)
# ===========================================================================


@app.command()
def sites():
    """List all available site IDs from the API."""
    try:
        resp = httpx.get(f"{get_api_base()}/sites", headers=get_auth_headers(), timeout=30.0)
...
def sync_sites(
    ...
    """
    try:
        resp = httpx.get(f"{get_api_base()}/sites/export", headers=get_auth_headers(), timeout=30.0)
        resp.raise_for_status()
    except httpx.HTTPStatusError as e:
        handle_http_error(e)
...
def reload_sites():
    ...
    try:
        resp = httpx.post(
            f"{get_api_base()}/api/sites/reset",
...
def links(
    ...
    try:
        resp = httpx.get(
            f"{get_api_base()}/sites/{site_id}/links",
            params=params,
            headers=get_auth_headers(),
            timeout=120.0,
        )
...
def content(
    ...
    try:
        resp = httpx.get(
            f"{get_api_base()}/sites/{site_id}/content",
            params=params,
            headers=get_auth_headers(),
            timeout=120.0,
        )
...
def index(
    ...
    try:
        resp = httpx.post(
            f"{get_api_base()}/sites/{site_id}/index",
...
def download(
    ...
    try:
        resp = httpx.get(
            f"{get_api_base()}/sites/{site_id}/download",
            headers=get_auth_headers(),
            timeout=120.0,
        )
        resp.raise_for_status()
...
def jobs(
    ...
    print("-" * 60)
    for j in jobs_list:
        job_sites = ", ".join(j.get("sites", [])[:3])
        if len(j.get("sites", [])) > 3:
            job_sites += f" +{len(j['sites']) - 3}"
        print(f"{j.get('job_id', '?'):<10} {j.get('status', '?'):<12} {j.get('progress', '?'):<12} {job_sites}")
...
def cache_stats():
    ...
    try:
        resp = httpx.get(f"{get_api_base()}/cache/stats", headers=get_auth_headers(), timeout=30.0)
        resp.raise_for_status()
    except httpx.HTTPStatusError as e:
        handle_http_error(e)
    data = resp.json()
...
def cache_keys(
    ...
    try:
        resp = httpx.get(
            f"{get_api_base()}/cache/keys", params=params, headers=get_auth_headers(), timeout=60.0
        )
        resp.raise_for_status()
    except httpx.HTTPStatusError as e:
        handle_http_error(e)
...
def cache_clear(
    ...
    try:
        resp = httpx.delete(f"{get_api_base()}/cache/{site_id}", headers=get_auth_headers(), timeout=30.0)
        resp.raise_for_status()
    except httpx.HTTPStatusError as e:
        handle_http_error(e)
